...
from iddm.config.choices import ldm_sample_choices, network_choices, optim_choices, act_choices, lr_func_choices, \
...
from iddm.model.trainers.ldm import LDMTrainer
...
def main(args):
    ...
    if args.distributed:
        ...
        mp.spawn(LDMTrainer(args=args).train, nprocs=gpus)
    else:
        LDMTrainer(args=args).train()
...
def init_train_args():
    ...
    parser.add_argument("--run_name", "-n", type=str, default="20250801-ldm-2")
    ...
    # Option: autoencoder
    parser.add_argument("--autoencoder_network", type=str, default="vae",
                        choices=autoencoder_network_choices)
    # Set the autoencoder checkpoint path (needed)
    parser.add_argument("--autoencoder_ckpt", type=str,
                        default="/your/path/of/autoencoder/weight.pt")
    ...
    # Input diffusion model image size (required)
    # Image size option: int or [height, width]
    # You can set 64, [64,64] or (64,64), the actual generated image size is 512*512
